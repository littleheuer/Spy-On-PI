...
int client_port, master_port, file_port, sig_port;
char port_temp[6] = {0}, master_ip[20];

void send_warning(char *buff) {
    if(NULL != strstr(buff, "warning")) {
        struct sockaddr_in addr;
        int w_fd = socket_udp_client(file_port, addr, master_ip);
        if(w_fd < 0) {
            perror("w_fd");
            close(w_fd);
            return ;
        }
        sendto(w_fd, buff, sizeof(buff), 0, (struct sockaddr *) &addr, sizeof(addr));
        close(w_fd);
    }
    return ;
}
    ...
    for(int i = 200; i <=205; i++) {
        ...
        while(find_file(dir, zipname, logname)) {
            if(strstr(zipname, ".gz") != NULL) {
                ...
                DBG("开始读取未压缩文件:%s\n", dirname);
                int data = 0;
                while ((data = fread(buffer, sizeof(buffer) - 1, 1000, fp)) > 0) {
                    //DBG("发送字符串:%s\n", buffer);
                    DBG("%d ", data);
                    send(file_fd, buffer, sizeof(buffer), 0);
                    fflush(stdout);
